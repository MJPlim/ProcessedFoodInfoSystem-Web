...
const Milk = (props) => {
    ...
    const [categoryName, setCategoryName] = useState('_육류');
    ...
    useEffect(() => {
        ...
        console.log(sort);
        setCategoryName('육류');
        if (sessionStorage.getItem('categoryName') === '_육류' || sessionStorage.getItem('categoryName') === null) {
            console.log('대분류를 눌럿음');
            sessionStorage.setItem('categoryName', '_육류');
    ...
    const getBigCategory = async (sort, pageNum, state) => {
        ...
        try {
            ...
            const { data } = await bigCategory.gotoCategory('육류', pageNum, sort, 12);
