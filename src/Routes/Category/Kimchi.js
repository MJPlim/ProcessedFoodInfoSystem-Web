...
const Kimchi = (props) => {
  ...
  const [categoryName, setCategoryName] = useState('_김치');
  ...
  useEffect(() => {
    ...
    console.log(sort);
    setCategoryName('김치');
    if (sessionStorage.getItem('categoryName') === '_김치' || sessionStorage.getItem('categoryName') === null) {
      console.log('대분류를 눌럿음');
      sessionStorage.setItem('categoryName', '_김치');
  ...
  const getBigCategory = async (sort, pageNum, state) => {
    ...
    try {
      ...
      const { data } = await bigCategory.gotoCategory('김치', pageNum, sort, 12);
