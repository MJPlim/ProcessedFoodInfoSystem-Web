...
const Etc = (props) => {
  ...
  const [categoryName, setCategoryName] = useState('_기타');
  ...
  useEffect(() => {
    ...
    console.log(sort);
    setCategoryName('기타');
    if (sessionStorage.getItem('categoryName') === '_기타' || sessionStorage.getItem('categoryName') === null) {
      console.log('대분류를 눌럿음');
      sessionStorage.setItem('categoryName', '_기타');
  ...
  const getBigCategory = async (sort, pageNum, state) => {
    ...
    try {
      ...
      const { data } = await bigCategory.gotoCategory('기타', pageNum, sort, 12);
