...
const Material = (props) => {
  ...
  const [categoryName, setCategoryName] = useState('_식재료');
  ...
  useEffect(() => {
    ...
    console.log(sort);
    setCategoryName('식재료');
    if (sessionStorage.getItem('categoryName') === '_식재료' || sessionStorage.getItem('categoryName') === null) {
      console.log('대분류를 눌럿음');
      sessionStorage.setItem('categoryName', '_식재료');
  ...
  const getBigCategory = async (sort, pageNum, state) => {
    ...
    setCategoryName('_식재료');
    sessionStorage.setItem('categoryName', '_식재료');

    try {
      state === 'init' && setLoading(true);

      const { data } = await bigCategory.gotoCategory('식재료', pageNum, sort, 12);
